  ...
  const updateFilter = (value: string) => {
    if (!Array.isArray(columnFilter)) setColumnFilter([value]);
      ...
      if ((columnFilter as Array<string>).includes(value)) {
        const newFilter = (columnFilter as Array<string>).filter(
          (v) => v != value
        );
    ...
    <Command loop>
      <CommandInput
        ...
        onKeyDown={(e) => {
          ...
          const searchResults = columnValuesCounted
            .filter(
              (v) =>
                commandScore(v[1].displayValue, inputValue ?? "", [
                  v[1].displayValue.toLowerCase(),
                ]) > 0
            )
            .map((v) => v[0]);
          ...
          if (e.key == "Enter")
            forceUpdate(
              searchResults.length > 0 &&
                searchResults.length != columnValuesCounted.length
                ? searchResults
                : undefined
            );
        }}
      ...
      <CommandList>
        ...
        <CommandItem
          ...
          onSelect={() => setColumnFilter(undefined)}
        >
          Supprimer les filtres
        </CommandItem>
        {columnValuesCounted.map((c) => {
          const content = c[0];
          return (
            <CommandItem
              key={content}
              value={c[1].displayValue}
              onSelect={() => updateFilter(content)}
            >
              <Checkbox
                className="mr-2"
                checked={
                  Array.isArray(columnFilter) &&
                  (columnFilter as Array<string>).includes(content)
                }
