...
export const DateFilterFunction = (
  row: any,
  columnId: string,
  filterValue: DateWithTimePickerType | DateRangeType
) => {
  ...
  return filterValueDate != undefined
    ? date.getDate() === filterValueDate.getDate() &&
        date.getMonth() === filterValueDate.getMonth() &&
        date.getFullYear() == filterValueDate.getFullYear() &&
        (filterTimePicker.hour !== true ||
          date.getHours() === filterValueDate.getHours()) &&
        (filterTimePicker.minute !== true ||
          date.getMinutes() === filterValueDate.getMinutes()) &&
        (filterTimePicker.second !== true ||
          date.getSeconds() === filterValueDate.getSeconds())
...
export default function FilterDate({
  ...
  const [timePicker, setTimePicker] = useState<TimePickerType>(
    columnFilter?.timePicker ?? { hour: false, minute: false, second: false }
  );
      ...
      <DropdownMenuItem {...disablingProps}>
        <Select
          onValueChange={(value) => {
            ...
            if (value == options.specific && selectedDate != undefined)
              setColumnFilter({ date: selectedDate, timePicker: timePicker });
            if (value == options.range && selectedRange != undefined)
              setColumnFilter(selectedRange);
        ...
        >
          ...
          <SelectContent>
            <SelectGroup>
              <SelectItem value={options.all}>Aucun filtre</SelectItem>
